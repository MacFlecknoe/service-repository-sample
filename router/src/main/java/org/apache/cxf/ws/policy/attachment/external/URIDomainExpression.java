...
public class URIDomainExpression implements DomainExpression {
	...
	public boolean appliesTo(ServiceInfo si) {
		...
		if ((si.getName() != null)
				&& wsdl11XPointer.matchesService(si.getTargetNamespace(), si
						.getName().getLocalPart())) {
			return true;
		}
		if ((si.getInterface() != null)
				&& (si.getInterface().getName() != null)
				&& wsdl11XPointer.matchesPortType(si.getTargetNamespace(), si
	...
	public boolean appliesTo(EndpointInfo ei) {
		...
		if ((ei.getService() != null)
				&& (ei.getService().getName() != null)
				&& (ei.getName() != null)
				&& wsdl11XPointer.matchesPort(ei.getService()
						.getTargetNamespace(), ei.getService().getName()
						.getLocalPart(), ei.getName().getLocalPart())) {
			return true;
		}
		return false;
	}
	...
	public boolean appliesTo(BindingOperationInfo boi) {
		...
		if ((boi.getBinding() != null)
				&& (boi.getBinding().getName() != null)
				&& wsdl11XPointer.matchesBinding(boi.getBinding().getName()
						.getNamespaceURI(), boi.getBinding().getName()
						.getLocalPart())) {
			return true;
		}
		if ((boi.getName() != null)
				...
				&& (boi.getBinding().getName() != null)
				&& wsdl11XPointer
						.matchesBindingOperation(boi.getBinding().getName()
								.getNamespaceURI(), boi.getBinding().getName()
								.getLocalPart(), boi.getName().getLocalPart())) {
			return true;
		}
		if ((boi.getOperationInfo() != null)
				...
				&& wsdl11XPointer.matchesPortTypeOperation(boi
						.getOperationInfo().getInterface().getName()
						.getNamespaceURI(), boi.getOperationInfo()
						.getInterface().getName().getLocalPart(), boi
	...
	public boolean appliesTo(BindingMessageInfo bmi) {
		...
		if ((bmi.getMessageInfo() != null)
				&& (bmi.getMessageInfo().getName() != null)
				&& wsdl11XPointer.matchesMessage(bmi.getMessageInfo().getName()
						.getNamespaceURI(), bmi.getMessageInfo().getName()
						.getLocalPart())) {
			return true;
		}
	...
	private boolean checkBindingOperationInOut(BindingMessageInfo bmi) {
		if ((bmi.getMessageInfo() != null)
			...
			if ((Type.INPUT == bmi.getMessageInfo().getType())
					&& wsdl11XPointer.matchesBindingOperationInput(bmi
							.getMessageInfo().getName().getNamespaceURI(), bmi
							.getBindingOperation().getBinding().getName()
							.getLocalPart(), bmi.getBindingOperation()
							.getName().getLocalPart())) {
				return true;
			}
			if ((Type.OUTPUT == bmi.getMessageInfo().getType())
					&& wsdl11XPointer.matchesBindingOperationOutput(bmi
							.getMessageInfo().getName().getNamespaceURI(), bmi
							.getBindingOperation().getBinding().getName()
							.getLocalPart(), bmi.getBindingOperation()
							.getName().getLocalPart())) {
	...
	private boolean checkPortTypeOperationInOut(BindingMessageInfo bmi) {
		...
		if ((ini != null) && (ini.getName() != null)
				...
				&& (bmi.getBindingOperation().getName() != null)) {
			if ((Type.INPUT == bmi.getMessageInfo().getType())
					&& wsdl11XPointer.matchesPortTypeOperationInput(ini
							.getName().getNamespaceURI(), ini.getName()
							.getLocalPart(), bmi.getBindingOperation()
							.getName().getLocalPart())) {
				return true;
			}
			if ((Type.OUTPUT == bmi.getMessageInfo().getType())
					&& wsdl11XPointer.matchesPortTypeOperationOutput(ini
							.getName().getNamespaceURI(), ini.getName()
							.getLocalPart(), bmi.getBindingOperation()
							.getName().getLocalPart())) {
				return true;
	...
	private boolean checkPortTypeOperationFault(BindingFaultInfo bfi) {
		...
		if ((ini != null)
				...
				&& wsdl11XPointer.matchesPortTypeOperationFault(ini.getName()
						.getNamespaceURI(), ini.getName().getLocalPart(), bfi
						.getBindingOperation().getName().getLocalPart(), bfi
						.getFaultInfo().getFaultName().getLocalPart())) {
			return true;
		}
		return false;
	}
	...
	private boolean checkBindingOperationFault(BindingFaultInfo bfi) {
		if ((bfi.getFaultInfo() != null)
				&& (bfi.getFaultInfo().getFaultName() != null)
				&& (bfi.getBindingOperation().getName() != null)
				&& (bfi.getBindingOperation().getBinding() != null)
				&& (bfi.getBindingOperation().getBinding().getName() != null)
				&& wsdl11XPointer.matchesBindingOperationFault(bfi
						.getFaultInfo().getFaultName().getNamespaceURI(), bfi
						.getBindingOperation().getBinding().getName()
						.getLocalPart(), bfi.getBindingOperation().getName()
						.getLocalPart(), bfi.getFaultInfo().getFaultName()
						.getLocalPart())) {
			return true;
		}
		return false;
	...
	public static class Wsdl11XPointer {
		...
		private static final Pattern PATTERN_DEFINITIONS = Pattern
				.compile("^wsdl11.definitions\\(\\)$");

		private static final Pattern PATTERN_PORT_TYPE = Pattern
				.compile("^wsdl11.portType" + SINGLE_PATH);

		private static final Pattern PATTERN_PORT_TYPE_OPERATION = Pattern
				.compile("^wsdl11.portTypeOperation" + DOUBLE_PATH);

		private static final Pattern PATTERN_PORT_TYPE_OPERATION_INPUT = Pattern
				.compile("^wsdl11.portTypeOperation.input" + DOUBLE_PATH);

		private static final Pattern PATTERN_PORT_TYPE_OPERATION_OUTPUT = Pattern
				.compile("^wsdl11.portTypeOperation.output" + DOUBLE_PATH);

		private static final Pattern PATTERN_PORT_TYPE_OPERATION_FAULT = Pattern
				.compile("^wsdl11.portTypeOperation.fault" + TRIPPLE_PATH);

		private static final Pattern PATTERN_MESSAGE = Pattern
				.compile("^wsdl11.message" + SINGLE_PATH);

		private static final Pattern PATTERN_MESSAGE_PART = Pattern
				.compile("^wsdl11.messagePart" + DOUBLE_PATH);

		private static final Pattern PATTERN_SERVICE = Pattern
				.compile("^wsdl11.service" + SINGLE_PATH);

		private static final Pattern PATTERN_PORT = Pattern
				.compile("^wsdl11.port" + DOUBLE_PATH);

		private static final Pattern PATTERN_BINDING = Pattern
				.compile("^wsdl11.binding" + SINGLE_PATH);

		private static final Pattern PATTERN_BINDING_OPERATION = Pattern
				.compile("^wsdl11.bindingOperation" + DOUBLE_PATH);

		private static final Pattern PATTERN_BINDING_OPERATION_INPUT = Pattern
				.compile("^wsdl11.bindingOperation.input" + DOUBLE_PATH);

		private static final Pattern PATTERN_BINDING_OPERATION_OUTPUT = Pattern
				.compile("^wsdl11.bindingOperation.output" + DOUBLE_PATH);

		private static final Pattern PATTERN_BINDING_OPERATION_FAULT = Pattern
				.compile("^wsdl11.bindingOperation.fault" + TRIPPLE_PATH);
		...
		boolean matchesPortTypeOperation(String wsdlTargetNamespace,
				String portType, String operation) {
			Matcher matcher = PATTERN_PORT_TYPE_OPERATION
					.matcher(wsdl11Pointer);
		...
		boolean matchesPortTypeOperationInput(String wsdlTargetNamespace,
				String portType, String operation) {
			Matcher matcher = PATTERN_PORT_TYPE_OPERATION_INPUT
					.matcher(wsdl11Pointer);
			return matches(wsdlTargetNamespace, matcher, portType, operation);
		}
		...
		boolean matchesPortTypeOperationOutput(String wsdlTargetNamespace,
				String portType, String operation) {
			Matcher matcher = PATTERN_PORT_TYPE_OPERATION_OUTPUT
					.matcher(wsdl11Pointer);
			return matches(wsdlTargetNamespace, matcher, portType, operation);
		}

		boolean matchesPortTypeOperationFault(String wsdlTargetNamespace,
				String portType, String operation, String fault) {
			Matcher matcher = PATTERN_PORT_TYPE_OPERATION_FAULT
					.matcher(wsdl11Pointer);
			return matches(wsdlTargetNamespace, matcher, portType, operation,
					fault);
		}
		...
		boolean matchesPort(String wsdlTargetNamespace, String service,
				String port) {
		...
		boolean matchesBindingOperation(String wsdlTargetNamespace,
		...
		boolean matchesBindingOperationInput(String wsdlTargetNamespace,
				String binding, String operation) {
			Matcher matcher = PATTERN_BINDING_OPERATION_INPUT
					.matcher(wsdl11Pointer);
			return matches(wsdlTargetNamespace, matcher, binding, operation);
		}
		...
		boolean matchesBindingOperationOutput(String wsdlTargetNamespace,
				String binding, String operation) {
			Matcher matcher = PATTERN_BINDING_OPERATION_OUTPUT
					.matcher(wsdl11Pointer);
			return matches(wsdlTargetNamespace, matcher, binding, operation);
		}

		boolean matchesBindingOperationFault(String wsdlTargetNamespace,
				String binding, String operation, String fault) {
			Matcher matcher = PATTERN_BINDING_OPERATION_FAULT
					.matcher(wsdl11Pointer);
			return matches(wsdlTargetNamespace, matcher, binding, operation,
					fault);
		}
